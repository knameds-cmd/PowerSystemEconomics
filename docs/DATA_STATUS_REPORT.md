...
| 3 | `generators.csv` | 9 | **1차 수정 완료** | KPG193 + EPSIS 발전기세부내역 + 연료단가 |
...
#### 1차 수정 (2026-04-09): KPG193 MATPOWER 데이터 반영

**변경 요약:**
- `pmin`: ~~경험적 비율 추정~~ → **KPG193 mpc.gen[Pmin] 클러스터 합산 x EPSIS 스케일 팩터**
- `pmax`: ~~EPSIS 단순 합산~~ → **KPG193 클러스터 합산 x EPSIS 스케일 팩터** (기존과 유사하나 클러스터 분할 정밀화)
- `ramp_up/down`: ~~Pmax 비율 추정~~ → **KPG193 mpc.genthermal[ramp_up/down] 클러스터 합산 x EPSIS 스케일 팩터**
- Coal 분할: ~~50/50 균등~~ → **KPG193 gencost MC@50%load 중위수 기준 저비용/고비용 분할 (21기/20기)**
- LNG CC/GT 분할: ~~60/40 추정~~ → **KPG193 Pmax>=400MW → CC, <400MW → GT로 분할 (55기/1기)**
...
| 클러스터 | 연료 | Pmax (MW) | Pmin (MW) | Pmin/Pmax | ramp_up (MW/h) | ramp/Pmax | MC (원/MWh) |
|---------|------|----------|----------|-----------|---------------|-----------|-----------|
| Nuclear_base | nuclear | 23,950 | 22,752 | 95.0% | 4,311 | 18.0% | 6,688 |
| Coal_lowcost | coal | 19,814 | 7,926 | 40.0% | 13,077 | 66.0% | 73,945 |
| Coal_highcost | coal | 18,094 | 7,238 | 40.0% | 11,942 | 66.0% | 81,297 |
| LNG_CC_low | lng | 21,591 | 8,205 | 38.0% | 21,591 | 100.0% | 140,526 |
| LNG_CC_mid | lng | 22,591 | 8,585 | 38.0% | 22,591 | 100.0% | 149,116 |
| CHP_mustrun | chp | 6,000 | 4,000 | 66.7% | 1,000 | 16.7% | 139,526 |
| LNG_GT_peak | lng | 118 | 45 | 38.1% | 118 | 100.0% | 207,244 |
| Oil_peak | oil | 2,501 | 0 | 0% | 2,501 | 100.0% | 427,801 |
| Hydro_fixed | hydro | 6,282 | 0 | 0% | 6,282 | 100.0% | 500 |
| **합계** | | **120,941** | **58,751** | | | | |

> 2024년 피크수요 97,115 MW 대비 예비율 약 24.5%
...
| `marginal_cost` | HR x 2024 실제 연료단가 + VOM | 높음 | 실데이터 연료단가 반영 |
| `must_run` | Nuclear/CHP = true, 나머지 false | 높음 | 운영 관행 기반 |
| `heat_rate` | DATA_SPEC 참조값 | 중간 | 전력시장운영규칙 데이터 확보 시 보완 가능 |
| `vom` | DATA_SPEC 참조값 | 중간 | 전력시장운영규칙 데이터 확보 시 보완 가능 |
| CHP 전체 | DATA_SPEC 기본값 | 낮음 | KPG193에 CHP 미포함, EPSIS에 별도 분류 없음 |
...
| 항목 | 현재 상태 | 필요한 데이터 |
|------|----------|-------------|
| `heat_rate` | DATA_SPEC 참조값 (교과서적) | 전력시장운영규칙 기준 클러스터별 가중평균 열소비율 |
| `vom` | DATA_SPEC 참조값 | 전력시장운영규칙 변동운영비 기준 |
| CHP 설비용량 | 6,000 MW 고정 추정 | CHP 발전기 별도 분류 정보 |
...
### 5.1 generators.csv 보완 (우선순위: 중간 — 1차 수정으로 주요 항목 해결)

1차 수정(KPG193 반영)으로 `pmin`, `pmax`, `ramp_up/down`, Coal/LNG 분할이 해결되었습니다.
남은 보완 항목:

| 보완 항목 | 현재 상태 | 필요한 데이터 소스 |
|----------|----------|-----------------|
| `heat_rate` | DATA_SPEC 참조값 | 전력시장운영규칙 클러스터별 가중평균 열소비율 |
| `vom` | DATA_SPEC 참조값 | 전력시장운영규칙 변동운영비 |
| CHP 전체 | 6,000 MW 고정 추정 | CHP 발전기 별도 분류 정보 |
| LNG_GT_peak | KPG193에 GT 1기만 (118MW, 과소) | 한국 GT 발전기 설비 목록 |
